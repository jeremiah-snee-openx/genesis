---
title: Architectural patterns (A1-A10)
description: The 10 Tier-3 architectural patterns. Each names a system shape -- macro topology of a non-trivial agentic capability -- and inherits the verbatim Genesis composition, anti-patterns, and selection rules.
---
...
A1 has its own [dedicated page](/genesis/reference/patterns/a1-panel/) -- the short version follows.
...
REAL EXAMPLE: the `apm-review-panel` skill in `microsoft/apm`. See [example 02 -- review panel architecture](/genesis/resources/examples/02-review-panel-architecture/) for the senior-engineer cautionary tale of getting this shape wrong.
...
> 3. After each wave, run the gate: do the wave's outputs satisfy the assumptions the next wave depends on?
...
STOP -- DECISION GATE BEFORE LANDING ON A6: A6 is the GENERAL event-reactive shape. If the request also names ANY of: `audit`, `auditable`, `compliance`, `sandbox`/`sandboxed`, `no-token`/`must not hold`, `capability-gating`, `governed`, then the more specific pattern [A10 GOVERNED OUTER LOOP](/genesis/reference/patterns/a10-governed-outer-loop/) applies and MUST be co-named (or selected over) A6. A10 inherits A6's event surface and adds substrate-enforced strong-form A9 + sandboxing + audit. Picking A6 alone in this case is an architectural error: it leaves the substrate guarantees the request explicitly asked for unrealized.
...
REAL EXAMPLE: the genesis README iteration loop -- specialist reviewers (genesis-expert, narrative-arc, funnel) plus four contrarian developer personas (newcomer, skeptic, power user, OSS maintainer) reading every draft cold. The cold readers caught positioning failures that the specialists rated GO. See [example 01 -- README iteration](/genesis/resources/examples/01-readme-iteration/) for the full walkthrough.
...
A10 has its own [dedicated page](/genesis/reference/patterns/a10-governed-outer-loop/) -- the short version follows.
...
See the [A10 dedicated page](/genesis/reference/patterns/a10-governed-outer-loop/) for the full WHEN clause, SUBSTRATE FIELDS, six anti-patterns, vendor-reality discussion, and the SELECTION HEURISTIC that distinguishes A10 from A6 + A9 alone.
...
- [Design patterns (Tier 2)](/genesis/reference/patterns/design/) -- the 23 patterns these architectural shapes compose.
- [Refactor patterns (Tier 4)](/genesis/reference/patterns/refactor/) -- apply BEFORE re-picking a Tier-3 shape.
- [A1 PANEL deep dive](/genesis/reference/patterns/a1-panel/), [A10 GOVERNED OUTER LOOP deep dive](/genesis/reference/patterns/a10-governed-outer-loop/), [B8 ATTENTION ANCHOR deep dive](/genesis/reference/patterns/b8-attention-anchor/).
- [Worked examples](/genesis/resources/examples/) -- five complete designs that name the Tier-3 shapes they realize.
- [Agentic SDLC Handbook, Chapter 12 -- Multi-Agent Orchestration](https://danielmeppiel.github.io/agentic-sdlc-handbook/handbook/ch12-multi-agent-orchestration.html).
